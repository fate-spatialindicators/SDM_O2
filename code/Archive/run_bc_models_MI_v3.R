...
dat <- load_bc_data(species.name = "sablefish", survey.name = "SYN WCHG")
# remove po2 outliers
  ...
  for(i in 1:length(m_df)){
    ...
    #formula <- gsub(pattern = "+ log_depth_scaled2 ",formula, replacement = "", fixed = T)
    ...
    if(use_cv==TRUE) {
      m <- sdmTMB_cv(
        ...
        spatial_only = TRUE,
        silent = TRUE)
      saveRDS(m, file = paste0("output/bc/cv/model_",i,"_MI_cv.rds"))
