...
dat <- load_data_nemuro(fit.model= F, spc = "sablefish", constrain_latitude=F)
# transform
...
wc_grid$depth <- -wc_grid$depth
wc_grid$log_depth_scaled <- scale(log(wc_grid$depth))
wc_grid$log_depth_scaled2 <- wc_grid$log_depth_scaled^2
# truncate limits based on haul filters
...
pred_po2 <- predict(po2_model,
                    ...
                    return_tmb_object = F)
# save to output
save(file = "output/wc_temp_nemuro.Rdata", pred_temp)
save(file = "output/wc_mi_nemuro.Rdata", pred_mi)
save(file = "output/wc_po2_nemuro.Rdata", pred_po2)
