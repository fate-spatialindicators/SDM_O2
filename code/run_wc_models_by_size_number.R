...
ggplot(us_coast_proj) + geom_sf() + 
  geom_point(data = dat, aes(x = X * 1000, y = Y * 1000, col = log(cpue_no_km2)), alpha = 0.6, size = 0.5) +
...
for (i in 1:length(m_df)) {
  ...
  formula = paste0("cpue_no_km2 ~ -1 +", m_df[i])
    m <- try(sdmTMB(
    ...
    if (class(m) != "try-error") {
        saveRDS(m, file = paste0("output/wc/model_", i, "_", spc,sizeclass,"number.rds"))
    }
}
...
if (use_AIC) {
  for (i in 1:length(m_df)) {
    ...
    if (!no_depth&!use_jscope)
      filename <- paste0("output/wc/model_", i, "_", spc,sizeclass, "number.rds")
    m <- readRDS(filename)
    AICmat[i, 1] <- AIC(m)
  }
}
...
best.index <- which(dAIC==0)
  filename <- paste0("output/wc/model_", best.index, "_", spc,sizeclass, "number.rds")
