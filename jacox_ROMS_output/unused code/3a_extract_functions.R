ncpath <- here::here("jacox_ROMS_output")
list.ncs <- list.files(ncpath, full.names=T)

########################################
########################################
########################################
## jameal making getDateRange function work with our data
########################################
########################################
########################################

### temp_bottom
filenames<-grep("temp_bottom_era5_monthly_1980_2010.nc",list.ncs)
fl<-list.ncs[filenames]
ptradius<-0.25
alt<-1
varname <- "temp"

inpts <- dat[,c('latitude_dd','longitude_dd','date')]
getDateRange(inpts=inpts,ncIn=fl)

########################################
########################################
########################################
## jameal making extract function work with our data
########################################
########################################
########################################
...
inpts <- dat
inpts$longitude_dd[1:5]

nc.data <- nc_open(nc, write=FALSE)
lat <- ncvar_get(nc.data,'lat')
lon <- ncvar_get(nc.data,'lon')
year <- ncvar_get(nc.data,'year') # added by js
month <- ncvar_get(nc.data,'month') # added by js
...
########################################
########################################
## from git repo https://github.com/elhazen/EcoCast-SciAdv/blob/master
...
getvar <- function(nc,varname,ptsfile,pt.radius,alt){
  ...
  if (varname=='sla'){  ### AVISO SLA SPECIFIC
    ...
    lat <- ncvar_get(nc.data,'lat')
    lon <- ncvar_get(nc.data,'lon')
    #tim <- ncvar_get(nc.data,'time') # removed by js
    year <- ncvar_get(nc.data,'year') # added by js
    month <- ncvar_get(nc.data,'month') # added by js
    #day <- as.Date(tim,origin='1950-01-01',tz= "UTC") # removed by js
