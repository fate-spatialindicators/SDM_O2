...
m_df <- formulas <- c("none",
          "temperature",
          "dissolved O2", 
          'pO2', 
          "MI", 
          "temperature + dissolved O2",
          "temperature + dissolved O2 + temperature x dissolved O2",
          "temperature + pO2",
          "temperature + pO2 + temperature x pO2",
          "breakpoint(dissolved O2)",
          "breakpoint(dissolved O2) + temperature",
          "breakpoint(pO2)",
          "breakpoint(pO2) + temperature",
          "breakpoint(MI)")
...
dAIC <- AICmat - min(AICmat)

dAICoutput <- data.frame(cbind(formulas, dAIC), stringsAsFactors= F)
dAICoutput[,2] <- as.numeric(dAICoutput[,2])
colnames(dAICoutput) <- c("Additional Predictors", '$\\Delta\\text{AIC}$')
knitr::kable(dAICoutput, digits = 2, row.names = F, longtable = FALSE)
}
```
...
m_mi <- readRDS("output/wc/model_14_MI.rds")
...
fitpars_po2 <- get_bp_parameters(m_po2)
bp_po2 <- fitpars_po2[1]; slope_po2 <- fitpars_po2[2]
bp_po2_true <- round(back.convert(bp_po2, mean.po2, std.po2),2)

fitpars_o2 <- get_bp_parameters(m_o2)
bp_o2 <- fitpars_o2[1]; slope_o2 <- fitpars_o2[2]
bp_o2_true <- round(back.convert(bp_o2, mean.o2, std.o2),2)


fitpars_mi <- get_bp_parameters(m_mi)
bp_mi <- fitpars_mi[1]; slope_o2 <- fitpars_mi[2]
bp_mi_true <- round(back.convert(bp_mi, mean.mi, std.mi),2)

fixed_effects_o2 <- m_o2$model$par
bp_ind <- grep("b_threshold", names(fixed_effects_o2))
bp_o2 <- fixed_effects_o2[bp_ind[2]]
slope_o2 <- fixed_effects_o2[bp_ind[1]]
bp_o2_true <- round(back.convert(bp_o2, mean.o2, std.o2),2)
...
ggplot(dat, aes(longitude, latitude, col = po2_bp)) + scale_colour_viridis_c() +
  geom_point(alpha=0.6, size = 0.5) + facet_wrap(~year) + labs(col = "p02 below breakpoint")
```


Unexpectedly, oxygen levels were greatest in shallow and nearshore areas compared to deeper areas (Figure \@ref(fig:plotoxygen2)).  There were some notable differences accross years, especially in 2011 when there were incursions of low oxygen water in the southern region of the sampling area.

```{r plotoxygen3, echo = F, fig.cap ="Spatial distribution of Sablefish"}


ggplot(dat, aes(longitude, latitude, col = log(cpue_kg_km2 ))) +  scale_colour_viridis_c()  +
  geom_point(alpha=0.6, size = 0.5) + facet_wrap(~year)

```


 There were no profound shifts in sablefish catch rates across the years, at least not based on simple maps (Figure \@ref(fig:plotoxygen3))




``` {r plotoxygen6, echo = F, fig.cap = "Fitted spatio-temporal model predicting metabolic index"}
mi_model <- readRDS("output/mi_model.rds")
...
``` {r plotoxygen7, echo = F, fig.cap = "Fitted spatio-temporal model predicting dissolved oxygen"}
o2_model <- readRDS("output/o2_model.rds")
ggplot(predict(o2_model), aes(longitude, latitude, col = exp(est))) + scale_colour_gradientn(colours = plasma(10)) +
  geom_point(alpha=0.6, size = 0.5) + facet_wrap(~year) + labs(col = "Fitted o2")

```

``` {r plotoxygen68, echo = F, fig.cap = "Fitted spatio-temporal model predicting pO2."}
po2_model <- readRDS("output/po2_model.rds")

ggplot(predict(po2_model), aes(longitude, latitude, col = exp(est))) + scale_colour_gradientn(colours = plasma(10)) +
  geom_point(alpha=0.6, size = 0.5) + facet_wrap(~year) + labs(col = "Fitted po2")
```


The fitted dissolved oxygen models indicated spatio-temporal variablity  (Figure \@ref(fig:plotoxygen7)).  Generally, 2015 had relatively high dissolved oxygen, especially in shallower areas.  In 2011, dissolved oxygen was lower in central Califronial than many other years.  Otherwise, dissolved oxygen was low along the deepest areas surveyed. 

The fitted metabolic index revealed generally high values throughout the sampling area, with spatially varying regions of low (<2) metabolic index across years (Figure \@ref(fig:plotoxygen6)).  There were few sites, however, with metabolic index values approaching 1 (the physiological threshold), likely because of the laboratory study upon which the metabolic index parameter was derived indicated that very low pO2 levels were needed to reach the critical threshold when demand exceeded supply.



``` {r make_breakpoint_table}

bp_output <- as.data.frame(cbind(c("dissolved oxygen", "pO2", "metabolic index"), c(bp_o2_true, bp_po2_true, bp_mi_true)))
colnames(bp_output) <- c("Predictor", "Breakpoint")
knitr::kable(x = bp_output, caption = "Estimated breakpoints for three different oxygen models", row.names = F)

```


The estimated breakpoints are indicated in the table below
...
dat$po2_effect <- dat$po2_effect / max(dat$po2_effect) # scaled to max = 1
ggplot(dat, aes(longitude, latitude, col = po2_effect)) + scale_colour_viridis_c() +
  geom_point(alpha=0.6, size = 0.5) + facet_wrap(~year)

```


The estimated effect of oxygen was relatively strong: namely the model predicts over a 50% reduction in density in areas with the lowest pO<sub>2</sub> 